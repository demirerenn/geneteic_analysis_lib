...
def scs(ss):
    """
    Verilen dizilerin en kısa ortak üst dizisini (Shortest Common Superstring) döndürür.
    Bu fonksiyon, hiçbir dizinin diğerinin alt dizisi olmadığını varsayar.
    Brute-force bir yaklaşımla tüm permütasyonları dener.

    Args:
        ss (list): Birleştirilecek DNA dizilerinin listesi.

    Returns:
        str: En kısa ortak üst dizi.
    """
    shortest_sup = None
    for ssperm in itertools.permutations(ss):
        sup = ssperm[0]
        for i in range(len(ss) - 1):
            # scs için overlap'i genellikle min_length=1 ile ararız
...
def pick_maximal_overlap(reads, k):
    """
    Verilen read listesinden, k'dan büyük veya eşit olan en uzun
    suffix/prefix overlap'ine sahip olan read çiftini döndürür.
    Eğer hiç overlap yoksa (None, None, 0) döndürür.
    """
...
def greedy_scs(reads, k):
    """
    Açgözlü (greedy) en kısa ortak üst dizi birleştirmesi yapar.
    En az k uzunluğunda bir overlap kalmayana kadar en iyi
    overlap'e sahip çiftleri birleştirmeyi tekrarlar.
    """
